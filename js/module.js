(function ($) {
  ...
  var initSidebar = function () {
    ...
    $(document).on(
      "tap.sidebar",
      ".sidebar, .pageHeader .menu-trigger",
      function (event) {
  ...
  var initWaypoints = function () {
    ...
    if (Response.band(1200)) {
      ...
      // Audience (B2B - B2C)
      $(".section-audience .container-template").waypoint(
        function () {
          $(this).addClass("in-point");
        },
        { offset: "80%" }
      ...
      // Centered Titles
      $(".section-intro h1, headergroup h2").waypoint(
        function () {
          $(this).addClass("in-point");
        },
        { offset: "85%" }
      ...
      $("section")
        .waypoint(
          function (direction) {
            ...
            $('.mainMenu a[href="#' + sectionName + '"]').toggleClass(
              "selected",
              direction === "down"
            );
          },
      ...
      $(
        ".section-intro h1, headergroup h2, .section-seperator, .timeline li, .rounded-icon, .section-audience .container-template"
      ).addClass("in-point");
    }
  };
  initWaypoints();
  ...
  var initParallax = function () {
    ...
    $(window)
      .on("throttledresize.parallax", function (event) {
        if (!Response.band(1200)) {
          $(".section-membership-section-seperator, .section-intro").addClass(
            "dont-scroll"
          );
        } else {
          $(
            ".section-membership-section-seperator, .section-intro"
          ).removeClass("dont-scroll");
        }
      })
      .trigger("throttledresize");
  };
  ...
  var initStatisics = function () {
    ...
    // Labels
    $(window)
      .on("throttledresize.pieChart", function (event) {
  ...
  var initProfileSwiper = function () {
    ...
    $(window)
      .on("throttledresize.profileSwiper", function (event) {
        if (Response.band(750)) {
          if (!profileSwiperInit) {
            profileSwiper =
              $(".profile-container").swiper(profileSwiperOptions);
  ...
  var initPricingSwiper = function () {
    var pricingSwiperOptions = {
        ...
        onSlideChangeStart: function (swiper) {
          ...
          $(".pricing-menu li:eq(" + currentSlide + ")")
            .addClass("selected")
            .siblings()
    ...
    // Enable only at 750px+
    $(window)
      .on("throttledresize.pricing", function (event) {
        if (Response.band(750)) {
          if (!pricingSwiperInit) {
            pricingSwiper =
              $(".pricing-container").swiper(pricingSwiperOptions);
